...
**作者**: OpenClaw 团队（🦞 Main + 💻 Coder + ✍️ Writer）
...
### 第四部分：认证与同步
- [第 7 章：认证配置](#第 7 章认证配置)
- [第 8 章：常见问题排查](#第 8 章常见问题排查)

### 第五部分：实例间通信
- [第 9 章：通信机制](#第 9 章通信机制)
- [第 10 章：任务下达方式](#第 10 章任务下达方式)
- [第 11 章：状态监控](#第 11 章状态监控)

### 第六部分：飞书集成
- [第 12 章：飞书应用配置](#第 12 章飞书应用配置)
- [第 13 章：飞书路由绑定](#第 13 章飞书路由绑定)
- [第 14 章：配对授权流程](#第 14 章配对授权流程)

### 第七部分：运维与管理
- [第 15 章：日常运维](#第 15 章日常运维)
- [第 16 章：配置验证清单](#第 16 章配置验证清单)

### 第八部分：实战案例
- [第 17 章：ClawClock 项目协作](#第 17 章 clawclock 项目协作)
- [第 18 章：配置错误修复实录](#第 18 章配置错误修复实录)
...
| 概念 | 说明 | 示例 |
|------|------|------|
| **实例 (Instance)** | 独立运行的 OpenClaw 进程 | Main、Coder、Writer |
| **Agent** | 实例内的 AI 代理 | main、coder、writer |
...
**命名规则**：
- 默认实例：不使用 `--profile`
- Profile 实例：`--profile coder`、`--profile writer`
...
**作用**：
- Main 实例可以访问 Coder 和 Writer 的会话
- Coder/Writer 不需要此配置（它们只执行任务）
...
sessions_send({
  sessionKey: "agent:coder:coder",
  message: `【任务】ClawClock 代码优化

项目路径：/home/lenovo/AItest/clawclock/
任务：
1. 重构 clock.py 中的冗余函数
2. 添加类型注解
3. 运行测试确保通过
...
#### 10.2 sessions_spawn（子代理）

**适用场景**：一次性任务

```javascript
sessions_spawn({
  task: "优化 ClawClock 项目的代码质量",
  agentId: "coder",
  mode: "run",
  streamTo: "parent"
})
...
```bash
# 查看所有实例状态
systemctl --user status openclaw-gateway.service openclaw-coder.service openclaw-writer.service

# 查看实时日志
journalctl --user -u openclaw-coder.service -f

# 查看会话文件数量
ls -la ~/.openclaw-coder/agents/main/sessions/*.jsonl | wc -l
# 输出：4 个会话
```
...
# 最近 1 小时日志
journalctl --user -u openclaw-coder.service --since "1 hour ago"
...
# 测试模型可用性
openclaw --profile coder agent -m "测试" --session-id test-123

# 检查路由
openclaw agents bindings

# 检查服务状态
systemctl --user status openclaw-writer.service
```
...
3. Coder → 自动回复 → Main: "代码完成"
4. Writer → 自动回复 → Main: "文档完成"
5. Main → 用户汇报：任务完成
```
...
openclaw agents bind --agent coder --bind feishu:coder-bot  # 添加路由
...
systemctl --user restart openclaw-coder.service  # 重启 Coder
journalctl --user -u openclaw-coder.service -f  # 实时日志
```
...
openclaw --profile coder models list          # 查看 Coder 的模型
```
...
**变量替换**：
- `{INSTANCE}`: Main/Coder/Writer
- `{PROFILE}`: 空（Main）/ `--profile coder` / `--profile writer`
- `{PORT}`: 18789 / 18989 / 18689
...
```bash
# 查看详细错误
journalctl --user -u openclaw-coder.service --since "5 minutes ago"

# 检查端口占用
ss -tlnp | grep 18989

# 检查配置文件
cat ~/.openclaw-coder/openclaw.json | jq .
```
